# Imposter Hunt Privacy Policy

**Effective: July 16, 2026**

Imposter Hunt is a local, offline party game for iPhone.

- The app has no accounts.
- The app does not collect, transmit, sell, share, or track any personal data.
- Player names, preferences, and custom words are stored locally on your device using UserDefaults.
- Deleting the app removes all locally stored data.
- For privacy questions, use the developer contact information on the App Store listing.

No data leaves the device. The app requires no network connection, login, or permissions beyond standard device features for gameplay.

Owner-controlled privacy policy URL and support URL fields in App Store Connect remain to be filled by the owner.